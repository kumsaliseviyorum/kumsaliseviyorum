  ...
  unsubscribeUsers = db.collection('users').orderBy('name').onSnapshot(snap => {
    ...
    snap.forEach(doc => {
      ...
      li.innerHTML = `
        ...
        <button class="btn btn-call" ${u.online ? '' : 'disabled'} data-uid="${uid}" data-name="${escapeHtml(u.name || '')}" aria-label="Ara">📞</button>
      `;
      memberListEl.appendChild(li);
    });

    if (memberListEl.innerHTML === '') {
      memberListEl.innerHTML = '<p style="color:var(--text-muted);font-size:13px;">Henüz başka aile üyesi katılmadı. Yakınlarını davet et!</p>';
    }
  });

  memberListEl.addEventListener('click', (e) => {
    const btn = e.target.closest('.btn-call');
    if (btn && !btn.disabled && typeof callUser === 'function') {
      callUser(btn.dataset.uid, btn.dataset.name);
    }
  });
}
