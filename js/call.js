...
const micOnIcon = muteBtn.querySelector('.icon-mic-on');
const micOffIcon = muteBtn.querySelector('.icon-mic-off');
...
function startCallTimer() {
  if (callTimerInterval) return; // zaten çalışıyor
...
function updateCallTimer() {
  ...
  const s = secs % 60;
  setCallStatus(`${m}:${s.toString().padStart(2, '0')}`);
}
...
muteBtn.addEventListener('click', () => {
  ...
  micOnIcon.style.display = track.enabled ? '' : 'none';
  micOffIcon.style.display = track.enabled ? 'none' : '';
});
...
async function callUser(uid, name) {
  ...
  activeCallLabel.textContent = name;
...
async function joinFamilyRoom() {
  ...
  activeCallLabel.textContent = 'Aile Odası';
  setCallStatus('Bağlanıyor…');
  participantGrid.innerHTML = '';
  activeCallEl.classList.remove('hidden');

  const existing = await db.collection('rooms').doc('family').collection('participants').get();
  existing.forEach(doc => {
    if (doc.id !== currentUser.uid) connectFamilyPeer(doc.id, doc.data().name);
  });

  await db.collection('rooms').doc('family').collection('participants').doc(currentUser.uid).set({
    name: currentUser.name,
    joinedAt: firebase.firestore.FieldValue.serverTimestamp()
  });
}

async function leaveFamilyRoom() {
  inFamilyRoom = false;
  joinRoomBtn.textContent = 'Odaya katıl';
  activeCallEl.classList.add('hidden');
  participantGrid.innerHTML = '';

  await db.collection('rooms').doc('family').collection('participants').doc(currentUser.uid).delete().catch(() => {});
...
function connectFamilyPeer(otherUid, otherName) {
  if (peerConnections[otherUid]) return; // zaten bağlı / bağlanıyor

  const pairId = [currentUser.uid, otherUid].sort().join('_');
  const amOfferer = currentUser.uid < otherUid;
  const connRef = db.collection('rooms').doc('family').collection('connections').doc(pairId);

  const pc = new RTCPeerConnection(ICE_SERVERS);
  peerConnections[otherUid] = { pc };
  addParticipantTile(otherUid, otherName);
  ...
  const myCandidates = amOfferer ? 'offerCandidates' : 'answerCandidates';
  const theirCandidates = amOfferer ? 'answerCandidates' : 'offerCandidates';

  pc.onicecandidate = (event) => {
    if (event.candidate) {
      connRef.collection(myCandidates).add(event.candidate.toJSON());
    }
  };
  ...
  if (amOfferer) {
    connRef.set({ from: currentUser.uid, to: otherUid }, { merge: true }).then(async () => {
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      await connRef.set({ offer: { type: offer.type, sdp: offer.sdp } }, { merge: true });
    });
  }

  connRef.onSnapshot(async (docSnap) => {
    const data = docSnap.data();
    if (!data) return;

    if (amOfferer && data.answer && !remoteDescSet) {
      remoteDescSet = true;
      await pc.setRemoteDescription(new RTCSessionDescription(data.answer));
    }

    if (!amOfferer && data.offer && !remoteDescSet) {
      remoteDescSet = true;
      await pc.setRemoteDescription(new RTCSessionDescription(data.offer));
      const answer = await pc.createAnswer();
      await pc.setLocalDescription(answer);
      await connRef.set({ answer: { type: answer.type, sdp: answer.sdp } }, { merge: true });
    }
  });

  connRef.collection(theirCandidates).onSnapshot(snap => {
    snap.docChanges().forEach(change => {
      if (change.type === 'added') {
        pc.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
      }
